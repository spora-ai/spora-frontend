 ...
 * `remove`). The shared `AgentProfilePictureSection.vue` thin-wraps
 * this for the agent pipeline; `GroupOverviewPage.vue` mounts it
 * directly with the group's store callbacks. State lives in this
 * component only as `pendingFile` / `saving` flags; every change goes
 * through the supplied callbacks so the caller's store owns the data.
 */
...
type CommitFn = (patch: ProfilePicturePatch) => Promise<void>
// eslint-disable-next-line no-unused-vars
...
async function pickArchetype(archetype: ArchetypeKey): Promise<void> {
  await commitPatch({ archetype })
}

async function cycleVariant(): Promise<void> {
  await commitPatch({ variant_key: nextVariant(currentVariant.value) })
}

async function pickPalette(palette: string): Promise<void> {
  await commitPatch({ palette_key: palette })
}
...
async function uploadImage(): Promise<void> {
  ...
  try {
    ...
    toast.success(`${sectionLabel.value} updated.`)
  } catch (e) {
