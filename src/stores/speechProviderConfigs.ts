 ...
 * Two cached lists (personal / global) so the admin page and the user
 * settings page can share one store instance without re-fetching.
 *
 * The backend's `GET /speech/provider-configs` returns both scopes in a
 * single envelope — admins see all global configs, callers see their
 * own user-scope overrides — and the getters slice that single array
 * for whichever view needs it. Cache invalidation is uniform: every
 * mutation calls `load()` so the next read sees the server's truth.
 */
