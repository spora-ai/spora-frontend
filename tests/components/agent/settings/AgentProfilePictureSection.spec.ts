...
describe('AgentProfilePictureSection', () => {
  ...
  it('groups archetype and palette buttons inside accessible fieldsets', () => {
    ...
    expect(archetypeFieldset?.querySelector('legend')?.textContent?.trim()).toBe('Archetype')
    ...
    expect(paletteFieldset?.querySelector('legend')?.textContent?.trim()).toBe('Palette')
  })
})
