/**
 * DashboardFilterChips — verifies that the chips render in order, that the
 * active chip is highlighted, and that clicking cycles through the chip
 * keys (with toggle-off behavior on the active chip). The Pinned and
 * Archived chips also disappear when no loaded agent carries the
 * corresponding flag.
 */
